...
def index_to_detail(index_url):
    ...
    index_sectors = start_response.xpath(
        '//*[@id="subPage"]//li[@class="list-item"][position()>1]/descendant::li')
...
def get_singer_data(singer_url, sleep_time, db, browser):
    ...
    try:
        for page in range(1, int(max_page[0])):
            next_button = browser.find_element_by_class_name(
                'page-navigator-next')
            next_button.click()
